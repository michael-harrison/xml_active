...
* <i>Record exists in XML but not in DB</i>: A new record in the DB is created
* <i>Record exists in DB but not in XML</i>: The DB Record is Destroyed
* <i>Record exists in XML and in DB</i>: DB record is replace if identifying features (eg id) don't match but if they do then the DB record is updated
...
* <b>Polymorphic Associations</b> Tested support these associations
